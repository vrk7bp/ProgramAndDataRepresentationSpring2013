//Venkata-Gautam Kanumuru, vrk7bp, February 26th, 2013, binarysearchtree.cpp
...
*/
{
  double returnValue = int_path_length(root, 0);
    return returnValue/num_nodes;  // stub, remove after writing your code
}
...
int BinarySearchTree::int_path_length(BinaryNode *t, int depth)
{
  ...
  if(t->left != NULL) {
    returnValue = returnValue + int_path_length(t->left, depth+1);
  }
  if(t->right != NULL) {
    returnValue = returnValue + int_path_length(t->right, depth+1);
  }
  return returnValue;
}
